...
export function AiProviderCredentialsSection({ rows, total, page, totalPages, pageSize, activeOnly, sort, loading, aliasSavingId, onSaveAlias, onOpenDetails, onPageChange, onPageSizeChange, onActiveOnlyChange, onSortChange, onConfigure }: AiProviderCredentialsSectionProps) {
  ...
  return (
    <CredentialSectionShell
      ...
      actions={onConfigure && (
        <Button type="button" variant="secondary" size="sm" appearance="action" onClick={onConfigure}>
          <IconSettings size={14} />
          {t('usage_stats.credentials_ai_providers_configure')}
        </Button>
      )}
    >
