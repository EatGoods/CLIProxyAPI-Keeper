...
describe('UsagePage CPAMC embed behavior', () => {
  ...
  it('uses CPA chrome and provider configuration while embedded', () => {
    expect(usagePageSource).toMatch(/\{!isEmbeddedInCPAMC && <header className=\{styles\.topBar\}>/);
    expect(usagePageSource).toContain('onConfigure={isEmbeddedInCPAMC ? openCPAMCAIProviders : undefined}');
  });
});
