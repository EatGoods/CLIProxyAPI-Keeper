...
func TestFetchPropagatesOpenAIProviderFieldsToEveryValidEntry(t *testing.T) {
	...
	entryNote := "first entry"
	// fetcher 默认其它六来源成功空列表。
	fetcher := successfulProviderFetcher()
	// OpenAI provider 故意缺 name，两个有效 entry 应共同使用默认名 openai。
	fetcher.openAIResult.Payload = []providerconfig.OpenAICompatibilityConfig{{Prefix: "shared-prefix", BaseURL: "https://shared.example/v1", Priority: &priority, Disabled: &disabled, Note: &note, APIKeyEntries: []providerconfig.OpenAIApiKeyEntry{{APIKey: "first-key", AuthIndex: "first-auth", Note: &entryNote}, {APIKey: "second-key", AuthIndex: "second-auth"}, {APIKey: "missing-auth"}}}}
	...
	for index, want := range []struct {
		...
		if credential.LookupKey != want.key || credential.AuthIndex != want.authIndex || credential.ProviderType != "openai" || credential.DisplayName != "openai" || credential.Prefix != "shared-prefix" || credential.BaseURL != "https://shared.example/v1" || credential.Priority != &priority || credential.Disabled != &disabled || credential.Note != wantNote {
