...
func TestOpenAICompatibilityConfigDecodesProviderAndEntryFields(t *testing.T) {
	...
	body := `{"id":"OpenRouter","prefix":"openrouter","base-url":"https://openrouter.ai/api/v1","priority":4,"disabled":true,"note":"shared","api-key-entries":[{"key":"first-key","auth_index":"first-auth","note":"first"},{"api-key":"second-key","auth-index":"second-auth"}]}`
	...
	if len(cfg.APIKeyEntries) != 2 || cfg.APIKeyEntries[0].APIKey != "first-key" || cfg.APIKeyEntries[0].AuthIndex != "first-auth" || cfg.APIKeyEntries[0].Note == nil || *cfg.APIKeyEntries[0].Note != "first" || cfg.APIKeyEntries[1].APIKey != "second-key" || cfg.APIKeyEntries[1].AuthIndex != "second-auth" {
