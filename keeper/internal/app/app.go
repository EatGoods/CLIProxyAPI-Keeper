...
func NewWithConfig(cfg config.Config) (*App, error) {
	...
	return newWithConfig(cfg, logCloser)
}

// NewEmbeddedWithConfig constructs Keeper without replacing the host process logger.
func NewEmbeddedWithConfig(cfg config.Config) (*App, error) {
	return newWithConfig(cfg, io.NopCloser(strings.NewReader("")))
}

func newWithConfig(cfg config.Config, logCloser io.Closer) (*App, error) {
